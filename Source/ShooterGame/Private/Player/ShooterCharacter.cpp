...
void AShooterCharacter::Tick(float DeltaSeconds)
{
	...
	if (LowHealthSound && GEngine->UseSound())
	{
		if ((this->Health > 0 && this->Health < this->GetMaxHealth() * LowHealthPercentage) && (!LowHealthWarningPlayer || !LowHealthWarningPlayer->IsPlaying()))
		{
			...
			AShooterCharacter* MyPlayer = MyPC ? Cast<AShooterCharacter>(MyPC->GetPawn()) : NULL;
			if (MyPlayer)
			{
				AShooterGameMode* GameMode = MyPlayer->GetWorld()->GetAuthGameMode<AShooterGameMode>();
				if (GameMode)
				{
					const FVector MyLoc = MyPlayer->GetActorLocation();
					...
					for (int32 i = 0; i < GameMode->LevelPickups.Num(); ++i)
					{
						...
						if (HealthPickup && HealthPickup->CanBePickedUp(MyPlayer))
						{
	...
	if (MyPC && this->CurrentWeapon->GetCurrentAmmo() < this->CurrentWeapon->GetAmmoPerClip())
	{
		AShooterCharacter* MyPlayer = MyPC ? Cast<AShooterCharacter>(MyPC->GetPawn()) : NULL;
		if (MyPlayer)
		{
			...
			if (GameMode)
			{
				...
				for (int32 i = 0; i < GameMode->LevelPickups.Num(); ++i)
				{
					...
					if (AmmoPickup && AmmoPickup->IsForWeapon(AShooterWeapon_Instant::StaticClass()) && AmmoPickup->CanBePickedUp(MyPlayer))
					{
