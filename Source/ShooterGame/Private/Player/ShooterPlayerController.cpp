...
void AShooterPlayerController::ManageDifficulty()
{
	if (this)
	{
		AShooterPlayerState* MyPlayerState = Cast<AShooterPlayerState>(this->PlayerState);
		int32 numKills = MyPlayerState->GetKills();
		int32 numDeaths = MyPlayerState->GetDeaths();

		int32 difficulty = 0;
		for (TObjectIterator<AShooterAIController> Itr; Itr; ++Itr)
		{
			difficulty = Itr->GetDifficulty();
			if (numKills - numDeaths > 2)
			{
				difficulty = Itr->GetDifficulty() + 1;
				difficulty = (difficulty > 5) ? 5 : difficulty;
				Itr->SetDifficulty(difficulty);
			}

			if (numDeaths >= numKills)
			{
				difficulty = Itr->GetDifficulty() - 1;
				difficulty = (difficulty < 1) ? 1 : difficulty;
				Itr->SetDifficulty(Itr->GetDifficulty() - 1);
			}
			break;
		}

		TArray<AShooterBot*> AliveBots;
		int32 numAlive = 0;

		for (TObjectIterator<AShooterAIController> Itr; Itr; ++Itr)
		{
			AShooterBot* Bot = Cast<AShooterBot>(Itr->GetPawn());
			if (Bot && Bot->IsAlive())
			{
				numAlive++;
				AliveBots.Add(Bot);
		...
		if (numAlive > difficulty && AliveBots.Num() > 0)
		{
			for (int32 i = 0; ((i < numAlive - difficulty) && (AliveBots.Num() > 0)); i++)
			{
				AShooterBot* Bot = AliveBots.Pop();
				Bot->Suicide();
			}
		}
	}
