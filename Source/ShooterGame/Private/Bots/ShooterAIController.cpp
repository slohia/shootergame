...
void AShooterAIController::Possess(APawn* InPawn)
{
	...
	if (Bot && Bot->BotBehavior)
	{
		...
		DifficultyKeyID = BlackboardComp->GetKeyID("Difficulty");
...
void AShooterAIController::BeginInactiveState()
{
	...
	const float MinRespawnDelay = (GameState && GameState->GameModeClass) ? GetDefault<AGameMode>(GameState->GameModeClass)->MinRespawnDelay : 5.0f;
...
int AShooterAIController::GetDifficulty()
{
	if (BlackboardComp)
	{
		return BlackboardComp->GetValue<UBlackboardKeyType_Int>(DifficultyKeyID);
	}
	return 0;
}

void AShooterAIController::SetDifficulty(int32 difficulty)
{
	if (BlackboardComp)
	{
		BlackboardComp->SetValue<UBlackboardKeyType_Int>(DifficultyKeyID, difficulty);
	}
}
